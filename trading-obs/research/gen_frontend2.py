...
<style>
:root{--bg:#080c10;--sur:#0d1218;--brd:#1a2230;--acc:#00e5ff;--grn:#7fff6e;--red:#ff4d6d;--ylw:#ffd166;--txt:#e2eaf4;--mut:#4a5a70}
...
.lbar{flex:1;min-width:3px;border-radius:1px 1px 0 0;background:var(--acc);opacity:.7}
.lbar.hi{background:var(--red);opacity:1}
...
.tr.spike-row{background:rgba(255,209,102,.06);border-left:2px solid var(--ylw)}
.tp{font-weight:700}.tp.b{color:var(--grn)}.tp.s{color:var(--red)}.tp.spk{color:var(--ylw)}
.tq{color:var(--mut);text-align:right}.tl{color:var(--mut);text-align:right;font-size:10px}.tl.sl2{color:var(--red)}
.spike-tag{font-size:9px;color:var(--ylw);text-align:right;grid-column:1/-1;padding:0 0 3px 0;opacity:.8}
...
#event-bar{padding:6px 16px;font-size:11px;background:rgba(255,209,102,.08);border-bottom:1px solid rgba(255,209,102,.2);color:var(--ylw);min-height:28px;display:flex;align-items:center;gap:8px;transition:opacity .5s}
#event-bar.empty{opacity:.3;color:var(--mut)}
.ev-dot{width:6px;height:6px;border-radius:50%;background:var(--ylw);flex-shrink:0}
</style>
</head>
<body>
...
<div class="mbar">
  ...
  <div class="mc"><div class="ml">Spikes</div><div class="mv" id="msk" style="color:var(--ylw)">0</div><div class="ms">last 5 min</div></div>
  <div class="mc"><div class="ml">Volume</div><div class="mv" id="mv2">-</div><div class="ms">last min</div></div>
</div>
<div id="event-bar" class="empty"><div class="ev-dot"></div><span id="ev-txt">No anomalies detected</span></div>
<div class="main">
  <div class="cp">
    <div class="ph">
...
<script>
var API='http://localhost:8000',sym='BTCUSDT',mins=5,prev=null,ch=null;
var SPIKE_THRESH=0.05;
...
function ic(){
  ...
  ch=new Chart(ctx,{
    ...
    data:{labels:[],datasets:[
      {data:[],borderColor:'#00e5ff',backgroundColor:'rgba(0,229,255,0.07)',borderWidth:2,pointRadius:0,tension:0.3,fill:true,label:'price'},
      {data:[],borderColor:'transparent',backgroundColor:'transparent',pointRadius:6,pointBackgroundColor:'#ffd166',pointBorderColor:'#ffd166',pointHoverRadius:8,showLine:false,label:'spikes'}
    ]},
    options:{
      ...
      plugins:{
        ...
        tooltip:{
          ...
          backgroundColor:'#0d1218',borderColor:'#1a2230',borderWidth:1,
          titleColor:'#4a5a70',bodyColor:'#e2eaf4',
          callbacks:{
            label:function(c){
              if(c.datasetIndex===0)return ' $'+c.parsed.y.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2});
              if(c.datasetIndex===1&&c.parsed.y!==null)return ' SPIKE: '+c.raw.note;
              return null;
            }
          }
        },
      ...
      scales:{
        x:{ticks:{color:'#4a5a70',maxTicksLimit:7,font:{size:10}},grid:{color:'#1a2230'}},
        y:{ticks:{color:'#4a5a70',font:{size:10},callback:function(v){return '$'+v.toLocaleString()}},grid:{color:'#1a2230'},position:'right'}
      },
...
function detectSpikes(data){
  var spikes=[];
  if(data.length<3)return spikes;
  var prices=data.map(function(d){return d.price});
  var diffs=[];
  for(var i=1;i<prices.length;i++)diffs.push(Math.abs(prices[i]-prices[i-1]));
  var avg=diffs.reduce(function(a,b){return a+b},0)/diffs.length;
  var std=Math.sqrt(diffs.map(function(d){return Math.pow(d-avg,2)}).reduce(function(a,b){return a+b},0)/diffs.length);
  var threshold=Math.max(avg+2*std, avg*2, SPIKE_THRESH);
  for(var i=1;i<data.length;i++){
    var diff=Math.abs(data[i].price-data[i-1].price);
    var pct=(diff/data[i-1].price)*100;
    if(diff>threshold&&pct>0.01){
      spikes.push({idx:i,price:data[i].price,diff:diff,pct:pct,time:data[i].event_time_ms,lat:Math.abs(data[i].latency_ms)});
    }
  }
  return spikes;
}
...
function uc(ev){
  ...
  var spikes=detectSpikes(d);
  var spikeMap={};
  spikes.forEach(function(sp){spikeMap[sp.idx]=sp});

  var labels=d.map(function(e){var dt=new Date(e.event_time_ms);return ('0'+dt.getHours()).slice(-2)+':'+('0'+dt.getMinutes()).slice(-2)+':'+('0'+dt.getSeconds()).slice(-2)});
  var prices=d.map(function(e){return e.price});
  var spikeData=d.map(function(e,i){
    if(spikeMap[i]){var sp=spikeMap[i];return {x:labels[i],y:e.price,note:(sp.diff>=0?'+':'-')+(sp.pct.toFixed(3))+'% in '+sp.lat+'ms'}}
    return {x:labels[i],y:null}
  });

  ch.data.labels=labels;
  ch.data.datasets[0].data=prices;
  ch.data.datasets[1].data=spikeData;
  ch.update('none');

  document.getElementById('msk').textContent=spikes.length;
  if(spikes.length>0){
    var last=spikes[spikes.length-1];
    var dir=last.diff>=0?'+':'-';
    var eb=document.getElementById('event-bar');
    eb.classList.remove('empty');
    document.getElementById('ev-txt').textContent='Price moved '+dir+last.pct.toFixed(3)+'% in '+last.lat+'ms — above normal range';
  }
}

async function fe(){var r=await fetch(API+'/events?symbol='+sym+'&limit=300');return r.json()}
async function fm(){var r=await fetch(API+'/metrics?symbol='+sym+'&minutes='+mins);return r.json()}
...
function um(ev,mx){
  ...
  if(mx.length){var m=mx[0];document.getElementById('mt').textContent=m.trade_count.toLocaleString();document.getElementById('mv2').textContent=m.volume.toFixed(3)}
}

function ulb(ls){
  var mx=Math.max.apply(null,ls)||1;
  document.getElementById('lb').innerHTML=ls.map(function(l){var h=Math.max(3,Math.round(l/mx*34));return '<div class="lbar'+(l>300?' hi':'')+'" style="height:'+h+'px"></div>'}).join('');
}

function uf(ev){
  if(!ev.length)return;
  var prices=ev.map(function(e){return e.price});
  var diffs=[];
  for(var i=1;i<prices.length;i++)diffs.push(Math.abs(prices[i]-prices[i-1]));
  var avg=diffs.length?diffs.reduce(function(a,b){return a+b},0)/diffs.length:0;
  var std=diffs.length?Math.sqrt(diffs.map(function(d){return Math.pow(d-avg,2)}).reduce(function(a,b){return a+b},0)/diffs.length):0;
  var threshold=Math.max(avg+2*std,0.01);

  document.getElementById('tf').innerHTML=ev.slice(0,80).map(function(e,i){
    var si=e.is_buyer_maker?'s':'b';
    var lt=Math.abs(e.latency_ms);
    var isSpike=i>0&&Math.abs(e.price-ev[i-1].price)>threshold;
    var pct=i>0?((e.price-ev[i-1].price)/ev[i-1].price*100).toFixed(3):null;
    var rowClass=isSpike?'tr spike-row':'tr';
    var priceClass=isSpike?'tp spk':'tp '+si;
    var tag=isSpike?'<div class="spike-tag">SPIKE '+(pct>=0?'+':'')+pct+'%</div>':'';
    return '<div class="'+rowClass+'"><div class="'+priceClass+'">'+e.price.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})+'</div><div class="tq">'+e.quantity.toFixed(5)+'</div><div class="tl'+(lt>300?' sl2':'')+' ">'+lt+'</div>'+tag+'</div>';
  }).join('');
}
...
function sw2(m,b){mins=m;document.querySelectorAll('.btn').forEach(function(x){x.classList.remove('on')});b.classList.add('on');document.getElementById('cr').textContent='last '+m+' min';rf()}
